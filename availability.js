...
export default function handler(req, res) {
  const { product, token } = req.query; // Získáme produkt a token z query parametru

  const validToken = "Un1Mark-7A9sKz!"; // Token pro ověření (měl by být bezpečně uložen)
  ...
  if (!product) {
    return res.status(400).json({ error: 'Product ID is required' }); // Pokud není předáno ID produktu
  }

  let availabilityData = {}; // Tady budou data o dostupnosti produktu a jeho lokalitách

  // Příklad pro produkt s ID 'product123'
